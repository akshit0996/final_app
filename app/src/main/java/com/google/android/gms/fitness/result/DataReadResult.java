...
public class DataReadResult implements Result, SafeParcelable {
    public static final Creator<DataReadResult> CREATOR = new C0341b();
    ...
    /* renamed from: a */
    public static DataReadResult m2713a(Status status, DataReadRequest dataReadRequest) {
    ...
    /* renamed from: a */
    private void m2714a(Bucket bucket, List<Bucket> list) {
        for (Bucket bucket2 : list) {
            if (bucket2.m2671b(bucket)) {
                for (DataSet a : bucket.getDataSets()) {
                    m2715a(a, bucket2.getDataSets());
                }
                return;
            }
        }
    ...
    /* renamed from: a */
    private void m2715a(DataSet dataSet, List<DataSet> list) {
        for (DataSet dataSet2 : list) {
            if (dataSet2.getDataSource().equals(dataSet.getDataSource())) {
                dataSet2.m2676a(dataSet.getDataPoints());
                return;
            }
        }
        list.add(dataSet);
    ...
    public boolean equals(Object that) {
        return this == that || ((that instanceof DataReadResult) && m2716c((DataReadResult) that));
    }
    ...
    public String toString() {
        return jv.m1571h(this).m1570a("status", this.Eb).m1570a("dataSets", this.Ua.size() > 5 ? this.Ua.size() + " data sets" : this.Ua).m1570a("buckets", this.Wr.size() > 5 ? this.Wr.size() + " buckets" : this.Wr).toString();
    }

    public void writeToParcel(Parcel dest, int flags) {
        C0341b.m651a(this, dest, flags);
    }
}
