...
public final class ZInvitationCluster implements SafeParcelable, Invitation {
    ...
    private void mp() {
        je.m1495K(!this.acE.isEmpty());
        Invitation invitation = (Invitation) this.acE.get(0);
        int size = this.acE.size();
        for (int i = 1; i < size; i++) {
            je.m1496a(invitation.getInviter().equals(((Invitation) this.acE.get(i)).getInviter()), "All the invitations must be from the same inviter");
        }
    }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        InvitationClusterCreator.m950a(this, dest, flags);
    }
}
