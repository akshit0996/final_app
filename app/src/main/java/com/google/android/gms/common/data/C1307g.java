...
public abstract class C1307g<T> extends DataBuffer<T> {
    private boolean Lr = false;
    private ArrayList<Integer> Ls;

    protected C1307g(DataHolder dataHolder) {
        super(dataHolder);
    }
        ...
        synchronized (this) {
            if (!this.Lr) {
                ...
                if (count > 0) {
                    ...
                    String ha = ha();
                    String c = this.JG.m2493c(ha, 0, this.JG.au(0));
                    int i = 1;
                    while (i < count) {
                        int au = this.JG.au(i);
                        String c2 = this.JG.m2493c(ha, i, au);
                        if (c2 == null) {
    ...
    protected int ay(int i) {
        ...
        String hc = hc();
        return (hc == null || this.JG.m2493c(hc, ax, au) != null) ? count : 0;
    }

    /* renamed from: f */
    protected abstract T mo3686f(int i, int i2);

    public final T get(int position) {
        hb();
        return mo3686f(ax(position), ay(position));
    }
