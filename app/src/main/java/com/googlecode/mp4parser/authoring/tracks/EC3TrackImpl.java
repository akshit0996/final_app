...
public class EC3TrackImpl extends AbstractTrack {
    ...
    public EC3TrackImpl(DataSource dataSource) throws IOException {
        ...
        for (BitStreamInfo bsi2 : this.bitStreamInfos) {
            if (bsi2.strmtyp == 1) {
                ...
                deps[i] = deps[i] + 1;
                chan_locs[bsi2.substreamid] = ((bsi2.chanmap >> 6) & 256) | ((bsi2.chanmap >> 5) & MotionEventCompat.ACTION_MASK);
            }
        }
    ...
    private BitStreamInfo readVariables() throws IOException {
        ...
        switch (entry.fscod) {
            case 0:
                entry.samplerate = VideoCreator.BUFFER_SIZE;
                break;
            case 1:
                entry.samplerate = SoundManager.REF_SAMPLE_RATE;
                break;
            case 2:
